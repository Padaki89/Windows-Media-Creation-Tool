@echo off
title Get Windows 11 on Unsupported PC || AveYo 2024.08.23
...
set MEDIA=.& set MOD=CLI& set PRE=WUA& set /a VER=11

if not defined CLI exit /b
if not exist "%SOURCES%\SetupHost.exe" exit /b
...
if %VER% == 11 (
    for %%W in ("%MEDIA%appraiserres.dll") do (
        if exist %%W if %%~zW == 0 set AlreadyPatched=1 & set /a VER=10
    )
)

if %VER% == 11 (
    findstr /r "P.r.o.d.u.c.t.V.e.r.s.i.o.n...1.0.\..0.\..2.[2-9]" "%SOURCES%\SetupHost.exe" >nul 2>nul || set /a VER=10
)

if %VER% == 11 if not exist "%MEDIA%EI.cfg" (
    echo;[Channel]>"%SOURCES%\EI.cfg"
    echo;_Default>>"%SOURCES%\EI.cfg"
)

if %VER%_%PRE% == 11_ISO (
    "%SOURCES%\WindowsUpdateBox.exe" /Product Server /PreDownload /Quiet %OPT%
    del /f /q "%SOURCES%\appraiserres.dll" 2>nul
    cd.>"%SOURCES%\appraiserres.dll"
    call :canary
)
...
:canary
:: Skip 2nd TPM check on Canary ISO
...
>nul reg add hkcu\software\classes\.Admin\shell\runas\command /f /ve /d "cmd /x /d /r set \"f0=%%2\"& call \"%%2\" %%3"
...
wmic /namespace:"\\root\subscription" path __EventFilter where Name="Skip TPM Check on Dynamic Update" delete >nul 2>nul
reg delete "%IFEO%\vdsldr.exe" /f 2>nul
